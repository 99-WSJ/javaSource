...
public abstract class Provider {
    ...
    public static javax.xml.ws.spi.Provider provider() {
        try {
            ...
            if (provider == null) {
                provider = javax.xml.ws.spi.FactoryFinder.find(JAXWSPROVIDER_PROPERTY, DEFAULT_JAXWSPROVIDER);
            }
            if (!(provider instanceof javax.xml.ws.spi.Provider)) {
                Class pClass = javax.xml.ws.spi.Provider.class;
    ...
    private static javax.xml.ws.spi.Provider getProviderUsingServiceLoader() {
        if (loadMethod != null) {
            Object loader;
            try {
                loader = loadMethod.invoke(null, javax.xml.ws.spi.Provider.class);
            } catch (Exception e) {
                throw new WebServiceException("Cannot invoke java.util.ServiceLoader#load()", e);
            }

            Iterator<javax.xml.ws.spi.Provider> it;
            try {
                it = (Iterator<javax.xml.ws.spi.Provider>)iteratorMethod.invoke(loader);
            } catch(Exception e) {

...
public class LogStream extends PrintStream {
    ...
    private static Map<String, java.rmi.server.LogStream> known = new HashMap<>(5);
    ...
    public static java.rmi.server.LogStream log(String name) {
        java.rmi.server.LogStream stream;
        synchronized (known) {
            stream = known.get(name);
            if (stream == null) {
                stream = new java.rmi.server.LogStream(name, defaultStream);
            }

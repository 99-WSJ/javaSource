...
public final class Collectors {
    ...
    public static<T,A,R,RR> Collector<T,A,RR> collectingAndThen(Collector<T,A,R> downstream,
        ...
        if (characteristics.contains(Collector.Characteristics.IDENTITY_FINISH)) {
            if (characteristics.size() == 1)
                characteristics = java.util.stream.Collectors.CH_NOID;
            else {
    ...
    Collector<T, ?, M> groupingBy(Function<? super T, ? extends K> classifier,
        ...
        BinaryOperator<Map<K, A>> merger = java.util.stream.Collectors.<K, A, Map<K, A>>mapMerger(downstream.combiner());
        @SuppressWarnings("unchecked")
    ...
    Collector<T, ?, M> groupingByConcurrent(Function<? super T, ? extends K> classifier,
        ...
        Supplier<A> downstreamSupplier = downstream.supplier();
        BiConsumer<A, ? super T> downstreamAccumulator = downstream.accumulator();
        BinaryOperator<ConcurrentMap<K, A>> merger = java.util.stream.Collectors.<K, A, ConcurrentMap<K, A>>mapMerger(downstream.combiner());
        @SuppressWarnings("unchecked")

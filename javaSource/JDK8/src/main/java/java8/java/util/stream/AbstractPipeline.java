...
abstract class AbstractPipeline<E_IN, E_OUT, S extends BaseStream<E_OUT, S>>
     ...
     * {@link StreamOpFlag}
     */
    AbstractPipeline(java.util.stream.AbstractPipeline<?, E_IN, ?> previousStage, int opFlags) {
    ...
    public S onClose(Runnable closeHandler) {
        ...
        sourceStage.sourceCloseAction =
                ...
                ? closeHandler
                : java.util.stream.Streams.composeWithExceptions(existingHandler, closeHandler);
        return (S) this;
    }
    ...
    private void parallelPrepare(int terminalFlags) {
        @SuppressWarnings("rawtypes")
        java.util.stream.AbstractPipeline backPropagationHead = sourceStage;
        if (sourceStage.sourceAnyStateful) {
            int depth = 1;
            for (@SuppressWarnings("rawtypes") java.util.stream.AbstractPipeline u = sourceStage, p = sourceStage.nextStage;
                 p != null;
                 u = p, p = p.nextStage) {
        ...
        if (terminalFlags != 0) {
            ...
            for (@SuppressWarnings("rawtypes") java.util.stream.AbstractPipeline p = backPropagationHead; p.nextStage != null; p = p.nextStage) {
    ...
    private Spliterator<?> sourceSpliterator(int terminalFlags) {
        ...
        if (isParallel()) {
            ...
            for (@SuppressWarnings("rawtypes") java.util.stream.AbstractPipeline u = sourceStage, p = sourceStage.nextStage, e = this;
                 u != e;
                 u = p, p = p.nextStage) {
    ...
    final StreamShape getSourceShape() {
        @SuppressWarnings("rawtypes")
        java.util.stream.AbstractPipeline p = java.util.stream.AbstractPipeline.this;
        while (p.depth > 0) {
            p = p.previousStage;
        }
    ...
    final <P_IN> Sink<P_IN> wrapSink(Sink<E_OUT> sink) {
        ...
        for (@SuppressWarnings("rawtypes") java.util.stream.AbstractPipeline p = java.util.stream.AbstractPipeline.this; p.depth > 0; p=p.previousStage) {

...
package java8.java.nio.file.spi;
...
public abstract class FileSystemProvider {
    ...
    // installed providers
    private static volatile List<java.nio.file.spi.FileSystemProvider> installedProviders;
    ...
    private static List<java.nio.file.spi.FileSystemProvider> loadInstalledProviders() {
        List<java.nio.file.spi.FileSystemProvider> list = new ArrayList<java.nio.file.spi.FileSystemProvider>();

        ServiceLoader<java.nio.file.spi.FileSystemProvider> sl = ServiceLoader
            .load(java.nio.file.spi.FileSystemProvider.class, ClassLoader.getSystemClassLoader());

        // ServiceConfigurationError may be throw here
        for (java.nio.file.spi.FileSystemProvider provider: sl) {
            String scheme = provider.getScheme();

            // add to list if the provider is not "file" and isn't a duplicate
            if (!scheme.equalsIgnoreCase("file")) {
                boolean found = false;
                for (java.nio.file.spi.FileSystemProvider p: list) {
    ...
    public static List<java.nio.file.spi.FileSystemProvider> installedProviders() {
        if (installedProviders == null) {
            ...
            java.nio.file.spi.FileSystemProvider defaultProvider = FileSystems.getDefault().provider();
            ...
            synchronized (lock) {
                if (installedProviders == null) {
                    ...
                    List<java.nio.file.spi.FileSystemProvider> list = AccessController
                        .doPrivileged(new PrivilegedAction<List<java.nio.file.spi.FileSystemProvider>>() {
                            @Override
                            public List<java.nio.file.spi.FileSystemProvider> run() {

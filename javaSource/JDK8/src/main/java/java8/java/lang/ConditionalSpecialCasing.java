...
//      new Entry(0x0130, new char[]{0x0069}, new char[]{0x0130}, "az", 0), // # LATIN CAPITAL LETTER I WITH DOT ABOVE
    ...
    static int toLowerCaseEx(String src, int index, Locale locale) {
        ...
        if (result != null) {
            if (result.length == 1) {
                ...
                return java.lang.Character.ERROR;
            }
        } else {
            // default to Character class' one
            return java.lang.Character.toLowerCase(src.codePointAt(index));
        }
    }
    ...
    static int toUpperCaseEx(String src, int index, Locale locale) {
        ...
        if (result != null) {
            ...
            // default to Character class' one
            return java.lang.Character.toUpperCaseEx(src.codePointAt(index));
        }
    }
    ...
    static char[] toUpperCaseCharArray(String src, int index, Locale locale) {
        ...
        if (result != null) {
            ...
            return java.lang.Character.toUpperCaseCharArray(src.codePointAt(index));
        }
    }
    ...
    private static boolean isFinalCased(String src, int index, Locale locale) {
        ...
        for (int i = index; (i >= 0) && !wordBoundary.isBoundary(i);
                i -= java.lang.Character.charCount(ch)) {
            ...
            if (isCased(ch)) {
                ...
                for (i = index + java.lang.Character.charCount(src.codePointAt(index));
    ...
    private static boolean isAfterI(String src, int index) {
        ...
        for (int i = index; i > 0; i -= java.lang.Character.charCount(ch)) {
    ...
    private static boolean isMoreAbove(String src, int index) {
        ...
        for (int i = index + java.lang.Character.charCount(src.codePointAt(index));
             i < len; i += java.lang.Character.charCount(ch)) {
    ...
    private static boolean isCased(int ch) {
        int type = java.lang.Character.getType(ch);
        if (type == java.lang.Character.LOWERCASE_LETTER ||
                type == java.lang.Character.UPPERCASE_LETTER ||
                type == java.lang.Character.TITLECASE_LETTER) {
            return true;
        } else {

...
public abstract class CommunicatorServer
    ...
    private transient Vector<com.sun.jmx.snmp.daemon.ClientHandler>
     ...
     */
    void notifyClientHandlerCreated(com.sun.jmx.snmp.daemon.ClientHandler h) {
     ...
     * For SNMP Runtime internal use only.
     */
    synchronized void notifyClientHandlerDeleted(com.sun.jmx.snmp.daemon.ClientHandler h) {
    ...
    private void terminateAllClient() {
        ...
        // this would most probably cause a deadlock.
        //
        final  com.sun.jmx.snmp.daemon.ClientHandler[] handlers =
                clientHandlerVector.toArray(new com.sun.jmx.snmp.daemon.ClientHandler[0]);
         for (com.sun.jmx.snmp.daemon.ClientHandler h : handlers) {
             try {
                 h.interrupt() ;
             } catch (Exception x) {
                 if (SNMP_ADAPTOR_LOGGER.isLoggable(Level.FINER)) {

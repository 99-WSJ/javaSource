...
package java8.sun.jmx.remote.util;
    ...
    public static int getNotifBufferSize(Map<String, ?> env) {
        ...
        try {
            if (env.containsKey(BUFFER_SIZE_PROPERTY)) {
                queueSize = (int) com.sun.jmx.remote.util.EnvHelp.getIntegerAttribute(env,BUFFER_SIZE_PROPERTY,
                                            defaultQueueSize,0,
                                            Integer.MAX_VALUE);
            } else { // try the old one
                queueSize = (int) com.sun.jmx.remote.util.EnvHelp.getIntegerAttribute(env,oldP,
                                            defaultQueueSize,0,
                                            Integer.MAX_VALUE);
            }
        } catch (RuntimeException e) {

...
final class SnmpRequestTree {
    ...
    static final class Enum implements Enumeration<SnmpMibSubRequest> {
        Enum(com.sun.jmx.snmp.agent.SnmpRequestTree hlist, Handler h) {
            handler = h;
            this.hlist = hlist;
            size = h.getSubReqCount();
        }
        private final Handler handler;
        private final com.sun.jmx.snmp.agent.SnmpRequestTree hlist;
        private int   entry = 0;
        private int   iter  = 0;
        private int   size  = 0;
    ...
    private void registerNode(SnmpMibNode meta, int depth, SnmpOid entryoid,
        ...
        if (meta == null) {
            SNMP_ADAPTOR_LOGGER.logp(Level.FINEST,
                    com.sun.jmx.snmp.agent.SnmpRequestTree.class.getName(),

...
abstract class MBeanIntrospector<M> {
    static final class PerInterfaceMap<M>
            extends WeakHashMap<Class<?>, WeakReference<com.sun.jmx.mbeanserver.PerInterface<M>>> {}
    ...
    abstract com.sun.jmx.mbeanserver.MBeanAnalyzer<M> getAnalyzer(Class<?> mbeanInterface)
    ...
    final com.sun.jmx.mbeanserver.PerInterface<M> getPerInterface(Class<?> mbeanInterface)
    throws NotCompliantMBeanException {
        PerInterfaceMap<M> map = getPerInterfaceMap();
        synchronized (map) {
            WeakReference<com.sun.jmx.mbeanserver.PerInterface<M>> wr = map.get(mbeanInterface);
            com.sun.jmx.mbeanserver.PerInterface<M> pi = (wr == null) ? null : wr.get();
            if (pi == null) {
                try {
                    com.sun.jmx.mbeanserver.MBeanAnalyzer<M> analyzer = getAnalyzer(mbeanInterface);
                    MBeanInfo mbeanInfo =
                            makeInterfaceMBeanInfo(mbeanInterface, analyzer);
                    pi = new com.sun.jmx.mbeanserver.PerInterface<M>(mbeanInterface, this, analyzer,
                            mbeanInfo);
                    wr = new WeakReference<com.sun.jmx.mbeanserver.PerInterface<M>>(pi);
    ...
    private MBeanInfo makeInterfaceMBeanInfo(Class<?> mbeanInterface,
            com.sun.jmx.mbeanserver.MBeanAnalyzer<M> analyzer) {
    ...
    private class MBeanInfoMaker
            implements com.sun.jmx.mbeanserver.MBeanAnalyzer.MBeanVisitor<M> {
     ...
     * per-interface data.
     */
    final MBeanInfo getMBeanInfo(Object resource, com.sun.jmx.mbeanserver.PerInterface<M> perInterface) {
        MBeanInfo mbi =
    ...
    final MBeanInfo getClassMBeanInfo(Class<?> resourceClass,
            com.sun.jmx.mbeanserver.PerInterface<M> perInterface) {
